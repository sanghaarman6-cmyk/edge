...
const theme = {
  appBg: 'bg-zinc-950',
  topbar: 'bg-white/60 backdrop-blur-xl border-b border-black/10',
...
function FloatingSidebar({
  ...
  return (
    <aside className="fixed left-4 top-1/2 z-50 -translate-y-1/2">
      ...
      >
        {/* Top logo */}
        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-black text-white text-lg font-semibold">
          PA
        </div>
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${active ? 'bg-black text-white' : 'text-white/70 hover:text-white'}
                `}
              >
                {item.icon}
        ...
        {/* Bottom action */}
        <button className="flex h-10 w-10 items-center justify-center rounded-xl text-white/60 hover:text-white">
          ⎋
        </button>
      </div>
    </aside>
...
function TopBar({
  ...
  return (
    <motion.header
      ...
      initial="hidden"
      animate="show"
      className={`${theme.topbar} sticky top-0 z-30`}
    >
      <div className="flex items-center justify-between px-5 py-4 md:px-7">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden text-sm text-zinc-500 sm:block">Status: Active</div>
          <div className="w-[140px]">
            <div className="h-2 w-full rounded-full bg-black/10">
              <div className="h-2 rounded-full bg-black" style={{ width: `${Math.min(100, Math.max(0, progress))}%` }} />
            </div>
            <div className="mt-1 text-right text-[11px] text-zinc-500">{progress}%</div>
          </div>
