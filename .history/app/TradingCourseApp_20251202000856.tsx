...
export default function TradingCourseApp() {
  ...
  const categories: Category[] = [
  {
    ...
    lessons: [
      { title: 'The Prop Firm Game — How the System Really Works', duration: '5:00', video: 'WGdUFP5vdt8', thumbnail: "/thumbnails/t1.png" },
      { title: 'The One-Trade Evaluation Pass Method', duration: '5:00', video: 'WGdUFP5vdt8', thumbnail: "/thumbnails/t2.png" },
      { title: 'The Funded Account Hedge System (Payout Engine)', duration: '5:00', video: 'WGdUFP5vdt8', thumbnail: "/thumbnails/t3.png" },
      { title: 'The Hedge Engine — Lot Sizes, Copier Setup & Margin Control', duration: '5:30', video: 'WGdUFP5vdt8', thumbnail: "/thumbnails/t4.png" },
      { title: 'The Ghost Protocol Playbook — Scaling, Ban Avoidance & Mastery', duration: '5:30', video: 'WGdUFP5vdt8', thumbnail: "/thumbnails/t5.png" },
      { title: 'Coming Soon...', duration: '00:00', video: null, thumbnail: "/thumbnails/t6.png" },
    ],
  },
];
...
const [lotPersonalLeverageInput, setLotPersonalLeverageInput] = useState<string>('500');
  ...
  const handleLotCalculate = () => {
    ...
    if (cfg.type === 'forex') {
      // For FX: pipValuePerLot = $10 per pip → contract ≈ 100k notional
      contractValuePerLot = 135000;
    }

    if (cfg.type === 'gold') {
      // Gold: $1 move = $100 per lot → contract = 100 oz × price
      const goldPrice =  XAUUSD_CURRENT_PRICE ?? 2400; 
      contractValuePerLot = 100 * goldPrice;
    }

    if (cfg.type === 'index') {
      // Indices: pipValuePerLot = $1 per point → contract = $1 × index price
      // Example NAS100 @ 18,000 → contract ~ $18,000
      const indexPrice = INDEX_CURRENT_PRICE ?? 18000;
      contractValuePerLot = indexPrice * 1;
    }
  ...
  // ✅ Main Course App
  return (
    <div className="flex h-screen bg-black text-white font-sans overflow-hidden overflow-y-visible">
      {/* Sidebar */}
      <AnimatePresence>
        {sidebarOpen && (
          <motion.div
            ...
            className="relative h-full bg-zinc-950 border-r border-zinc-800 flex flex-col justify-between py-6 z-20 shadow-lg"
          >
            <div>
              <div className="w-full flex items-center justify-center px-6 mb-6">
            ...
            <button
              ...
              className="absolute top-1/2 -translate-y-1/2 right-0 z-30 flex flex-col items-center justify-center h-16 w-4 group"
              title="Close sidebar"
            >
              <div className="w-px h-full bg-zinc-700 group-hover:bg-blue-400 transition-colors duration-300"></div>
              <span className="absolute text-zinc-500 group-hover:text-blue-400 text-lg font-bold -left-2">
                ❮
              </span>
            </button>
          </motion.div>
      ...
      {!sidebarOpen && (
        <button
          ...
          className="fixed top-1/2 -translate-y-1/2 left-0 z-30 flex flex-col items-center justify-center h-16 w-4 group"
          title="Open sidebar"
        >
          <div className="w-px h-full bg-zinc-700 group-hover:bg-blue-400 transition-colors duration-300"></div>
          <span className="absolute text-zinc-500 group-hover:text-blue-400 text-lg font-bold -right-2">
            ❯
          </span>
        </button>
      )}
      ...
      <motion.div
        ...
        className="flex-1 p-8 overflow-y-auto h-full mt-4 sm:mt-6"
      >
        {/* 🏠 HOME */}
        {activeTab === 'home' && (
          <div className="flex flex-col items-center justify-start w-full space-y-8">
            ...
            {/* 🎬 Sleek Intro Video (no border, with top mask) */}
            <div className="relative w-full max-w-4xl rounded-xl overflow-hidden shadow-[0_0_15px_rgba(0,0,0,0.4)]">
              <div className="pointer-events-none absolute inset-x-0 top-0 h-10 bg-gradient-to-b from-black/90 to-transparent z-20" />
              <LessonPlayer videoId="WGdUFP5vdt8" />
            </div>
          </div>
        )}
        ...
        {activeTab === 'lessons' && (
          <div className="w-full">
            ...
            >
              ...
              <div className="flex gap-6 px-2 overflow-visible">
...
/** Stable + crash-proof Plyr wrapper */
function LessonPlayer({
  ...
  const playerRef = useCallback(
    (instance: APITypes | null) => {
      if (!instance || !onComplete) return;
      const api = instance as unknown as { plyr?: PlyrInstance };
      const plyr = api.plyr;

      // If plyr is not ready yet — DO NOTHING
      if (!plyr || !plyr.elements || !plyr.elements.container) return;

      // Prevent double-binding
      if ((plyr as any).__attached) return;
      (plyr as any).__attached = true;
  ...
  return (
    <Plyr
      // @ts-ignore
      ref={playerRef}
      source={{ type: 'video', sources: [{ src: videoId, provider: 'youtube' }] }}
      options={{
