...
function useWistiaMedia(wistiaId: string | null) {
  useEffect(() => {
    if (!wistiaId) return;

    const id = `wistia-jsonp-${wistiaId}`;
    if (document.getElementById(id)) return;

    const s = document.createElement("script");
    s.id = id;
    s.src = `https://fast.wistia.com/embed/medias/${wistiaId}.jsonp`;
    s.async = true;
    document.body.appendChild(s);

    return () => {
      // optional cleanup (usually fine to keep cached)
...
export default function TrainingLandingPage() {
  ...
  useWistiaMedia(
  activeVideo?.type === 'wistia' ? activeVideo.wistiaId : null
);
...
const TESTIMONIAL_VIDEOS = [
  ...
  },
  {
    wistiaId: 'qeq4q1kihg',
    name: 'Dennis',
    result: 'Funded In One Day - 1 Step Eval',
              ...
              >
                {[...VIDEOS, ...VIDEOS].map((v, i) => (
                  <div
                    ...
                    onClick={() => setActiveVideo({ type: 'youtube', id: v.id })}
...
<section className="w-full max-w-6xl mt-16">
  <div className="text-center mb-10">
    ...
    <p className="text-zinc-400 mt-3">
      Life changing lessons. Real experiences.
    </p>
  </div>
  ...
  <div className="flex md:grid md:grid-cols-2 gap-6 overflow-x-auto md:overflow-visible scrollbar-hide snap-x">
    {TESTIMONIAL_VIDEOS.map((v, i) => (
      <div
        ...
        onClick={() => {
  setActiveVideo({ type: 'wistia', wistiaId: v.wistiaId });
}}
      >
        <div className="relative aspect-video rounded-2xl overflow-hidden border border-zinc-800 bg-black">
          ...
          <div className="absolute bottom-4 left-4">
            ...
            <p className="text-xs text-zinc-300">{v.result}</p>
          </div>
        </div>
      </div>
    ))}
...
{activeVideo?.type === 'wistia' && (
  <div
    key={activeVideo.wistiaId}
    className={`wistia_embed wistia_async_${activeVideo.wistiaId}`}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-8 w-full max-w-md relative">
            ...
            <form
                ...
                // ✅ REDIRECT AFTER SAVE
                window.location.href = '/join/watch';
              }}
